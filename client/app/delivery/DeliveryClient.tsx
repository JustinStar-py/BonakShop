...
const orderStatusMap = {
  PENDING: { text: "در حال بررسی", variant: "secondary" },
  SHIPPED: { text: "ارسال شده", variant: "default" },
  DELIVERED: { text: "تحویل داده شد", variant: "default" },
  CANCELED: { text: "لغو شده", variant: "destructive" },
} as const;

const returnStatusMap = {
  REQUESTED: { text: "درخواست شده", variant: "secondary" },
  APPROVED: { text: "تایید شده", variant: "default" },
  REJECTED: { text: "رد شده", variant: "destructive" },
} as const;

const getOrderStatusInfo = (status: OrderStatus) => orderStatusMap[status] ?? orderStatusMap.PENDING;
const getReturnStatusInfo = (status: ReturnStatus) => returnStatusMap[status] ?? returnStatusMap.REQUESTED;
...
export default function DeliveryClient({
  ...
  initialOrders?: OrderForDelivery[];
  initialReturns?: ReturnForDelivery[];
}) {
  const { user, setUser, isLoadingUser } = useAppContext();
  const router = useRouter();
  const hasInitialData =
    typeof initialOrders !== "undefined" && typeof initialReturns !== "undefined";
  const [allOrders, setAllOrders] = useState<OrderForDelivery[]>(initialOrders ?? []);
  const [allReturns, setAllReturns] = useState<ReturnForDelivery[]>(initialReturns ?? []);
  ...
  useEffect(() => {
    if (!isLoadingUser && (!user || (user.role !== "WORKER" && user.role !== "ADMIN"))) {
      router.replace("/");
    }
  ...
  }, [user, isLoadingUser, router, hasInitialData, fetchData]);
        ...
        <div>
          {user.role === "ADMIN" && (
            <Button
              ...
              className="ml-2"
            >
              <ArrowRight className="ml-2 h-4 w-4" />داشبورد
            </Button>
          )}
...
function OrdersPanel({
  ...
  const ordersByStatus = useMemo(() => {
    ...
    const others = orders.filter(
      (o: OrderForDelivery) => o.status !== "SHIPPED" && o.status !== "DELIVERED"
    );
  ...
  const OrderCard = ({ order }: { order: OrderForDelivery }) => (
    <Card className="flex flex-col" dir="rtl">
      <CardHeader>
        ...
        <CardDescription>{new Date(order.createdAt).toLocaleDateString("fa-IR")}</CardDescription>
        <div className="space-y-1.5 text-sm text-muted-foreground pt-2">
          <p className="flex items-center gap-2">
            <UserIcon className="h-4 w-4" />{order.user.name}
          </p>
          <p className="flex items-start gap-2">
            <Building className="h-4 w-4 mt-1 shrink-0" />{order.user.shopAddress}
          </p>
          <p className="flex items-center gap-2">
      ...
      <CardContent className="flex flex-col gap-2 mt-auto">
        <Dialog>
          <DialogTrigger asChild>
            <Button variant="outline" className="w-full">
              <FileText className="ml-2 h-4 w-4" />مشاهده فاکتور
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-sm">
            <CardHeader>
              <CardTitle>فاکتور سفارش</CardTitle>
              <CardDescription>شماره: ...{order.id.slice(-6)}</CardDescription>
            </CardHeader>
        ...
        {order.user.latitude && order.user.longitude && (
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" className="w-full">
                <MapPin className="ml-2 h-4 w-4" />نمایش روی نقشه
              </Button>
            </DialogTrigger>
            <DialogContent>
              <MapPicker
                readOnly
                marker={{
                  position: [order.user.latitude, order.user.longitude],
                  popupText: order.user.shopName || "موقعیت",
                }}
              />
            </DialogContent>
          </Dialog>
  ...
  return (
    <Tabs defaultValue="shipped" className="w-full">
      ...
      <TabsContent value="shipped" className="pt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {ordersByStatus.shipped.map((order) => <OrderCard key={order.id} order={order} />)}
      </TabsContent>
      <TabsContent value="delivered" className="pt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {ordersByStatus.delivered.map((order) => <OrderCard key={order.id} order={order} />)}
      </TabsContent>
      <TabsContent value="others" className="pt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {ordersByStatus.others.map((order) => <OrderCard key={order.id} order={order} />)}
      </TabsContent>
    </Tabs>
  );
}
  ...
  const ReturnCard = ({ ret }: { ret: ReturnForDelivery }) => (
    <Card>
      <CardHeader>
        <div className="flex justify-between items-start">
          <CardTitle className="text-base font-bold">
            {ret.order.user.shopName || ret.order.user.name}
          </CardTitle>
          <Badge variant={getReturnStatusInfo(ret.status).variant}>
            {getReturnStatusInfo(ret.status).text}
          </Badge>
        </div>
        <CardDescription>
      ...
      <CardContent>
        <div className="text-sm space-y-2">
          ...
          <ul className="list-disc list-inside bg-gray-50 p-2 rounded-md">
            {ret.items.map((item) => (
              <li key={item.id}>{item.orderItem.productName} (تعداد: {item.quantity})</li>
            ))}
          </ul>
          {ret.reason && (
            <p>
              <span className="font-semibold">دلیل:</span> {ret.reason}
            </p>
          )}
        </div>
        <div className="flex items-center gap-2 mt-4">
          <Select
            onValueChange={(value) => handleStatusChange(ret.id, value as ReturnStatus)}
