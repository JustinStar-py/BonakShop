...
export default function OrdersClient({
  ...
  const hasInitialData = typeof initialOrders !== "undefined";
  const [orders, setOrders] = useState<OrderWithItems[]>(initialOrders ?? []);
  const [isLoading, setIsLoading] = useState(!hasInitialData && !initialError);
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(fetchError, "خطا در دریافت سفارشات."));
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  }, [fetchOrders, hasInitialData, initialError]);
  ...
  const handleCancelOrder = async (orderId: string) => {
    try {
      ...
      alert(getErrorMessage(cancelError, "خطا در لغو سفارش"));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] p-4">
        ...
        <p className="text-gray-400 text-sm mb-6 text-center">
          محصولات متنوع ما را مشاهده کنید و اولین سفارش خود را ثبت کنید.
        </p>
        <Button
          className="bg-green-600 text-white rounded-xl h-12 px-8"
          onClick={() => router.push("/")}
        >
          شروع خرید
        </Button>
                  ...
                  {order.items.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <TomanPrice value={item.price} className="text-xs font-medium" />
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            آیا اطمینان دارید؟ این عملیات قابل بازگشت نیست.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
