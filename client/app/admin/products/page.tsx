// FILE: app/admin/products/page.tsx (FINAL & COMPLETE CODE)
"use client";
...
import { Loader2, Pencil, PlusCircle, Trash2, Upload, Star } from "lucide-react";
...
export default function ProductManagementPage() {
  ...
  // State for the nested "add entity" dialog
  ...
  const handleSelectChange = (field: EntityType, value: string) => {
      const addValueMap = {
          ...
          'distributor': 'add-new-distributor'
      };
      if (value === addValueMap[field]) {
          setAddEntity({ type: field, isOpen: true });
      } else {
  ...
  const handleAddNewEntity = async () => {
      ...
      const apiMap = {
          'category': { path: '/categories', key: 'categoryId' },
          'supplier': { path: '/suppliers', key: 'supplierId' },
          'distributor': { path: '/distributors', key: 'distributorId' }
      };
      ...
      try {
          ...
          setAddEntity({ type: null, isOpen: false });
          setNewEntityName("");
      } catch (error) {
          alert(`خطا در افزودن آیتم جدید: ${error}`);
      } finally {
  ...
  return (
    <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-4 max-h-[80vh] overflow-y-auto p-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Label>دسته‌بندی</Label>
                    <Select value={editingProduct.categoryId} onValueChange={val => handleSelectChange('category', val)} required>
                        <SelectTrigger><SelectValue placeholder="انتخاب..." /></SelectTrigger>
                        <SelectContent>
                            {categories.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                            <SelectItem value="add-new-category" className="text-blue-500 font-bold">افزودن دسته‌بندی جدید</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                <div><Label>تولیدکننده</Label>
                    <Select value={editingProduct.supplierId} onValueChange={val => handleSelectChange('supplier', val)} required>
                        <SelectTrigger><SelectValue placeholder="انتخاب..." /></SelectTrigger>
                        <SelectContent>
                            {suppliers.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}
                            <SelectItem value="add-new-supplier" className="text-blue-500 font-bold">افزودن تولیدکننده جدید</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                <div><Label>پخش‌کننده</Label>
                    <Select value={editingProduct.distributorId} onValueChange={val => handleSelectChange('distributor', val)} required>
                        <SelectTrigger><SelectValue placeholder="انتخاب..." /></SelectTrigger>
                        <SelectContent>
                            {distributors.map(d => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}
                            <SelectItem value="add-new-distributor" className="text-blue-500 font-bold">افزودن پخش‌کننده جدید</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
      ...
      {/* Nested Dialog for adding Category/Supplier/Distributor */}
      <Dialog open={addEntity.isOpen} onOpenChange={() => setAddEntity({ type: null, isOpen: false })}>
          <DialogContent dir="rtl" className="sm:max-w-md z-[60]">
              <DialogHeader>
                  <DialogTitle>افزودن 
                      {addEntity.type === 'category' && ' دسته‌بندی جدید'}
                      {addEntity.type === 'supplier' && ' تولیدکننده جدید'}
                      {addEntity.type === 'distributor' && ' پخش‌کننده جدید'}
                  </DialogTitle>
              </DialogHeader>
              ...
              <DialogFooter>
                  ...
                  <Button onClick={handleAddNewEntity} disabled={actionLoading || !newEntityName}>
