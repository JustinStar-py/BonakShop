...
function ProductManagementPage() {
    ...
    const [newProduct, setNewProduct] = useState({ name: "", price: "", description: "", categoryId: "", image: "", available: true });
    const [actionLoading, setActionLoading] = useState<string | null>(null);
    ...
    const handleProductSubmit = async (e: FormEvent) => { e.preventDefault(); setActionLoading('new'); try { await fetch('/api/products', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...newProduct, price: parseFloat(newProduct.price) }) }); await fetchProductsAndCategories(); (document.getElementById('add-product-close') as HTMLButtonElement)?.click(); setNewProduct({ name: "", price: "", description: "", categoryId: "", image: "", available: true }); } catch (e) { alert("خطا در افزودن محصول"); } finally { setActionLoading(null); } };
    const handleProductUpdate = async (e: FormEvent) => { e.preventDefault(); if (!editingProduct) return; setActionLoading(editingProduct.id); try { await fetch(`/api/products/${editingProduct.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...editingProduct, price: parseFloat(editingProduct.price as any) }) }); await fetchProductsAndCategories(); setEditingProduct(null); } catch (e) { console.error(e); alert("خطا در ویرایش محصول"); } finally { setActionLoading(null); } };
    const handleToggleAvailability = async (product: Product) => { setActionLoading(product.id); try { await fetch(`/api/products/${product.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ available: !product.available }) }); await fetchProductsAndCategories(); } catch (error) { console.error(error); alert("خطا در تغییر وضعیت محصول."); } finally { setActionLoading(null); } };
    
    if (isLoading) return <p className="text-center py-10">در حال بارگذاری...</p>;

    return (
        <div>
            <div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold">مدیریت محصولات</h1><Dialog><DialogTrigger asChild><Button><PlusCircle className="ml-2 h-4 w-4" />افزودن محصول</Button></DialogTrigger><DialogContent dir="rtl" className="sm:max-w-[425px]"><DialogHeader><DialogTitle>افزودن محصول جدید</DialogTitle></DialogHeader><form onSubmit={handleProductSubmit} className="space-y-4 pt-4"><div><Label>نام</Label><Input value={newProduct.name} onChange={e => setNewProduct({...newProduct, name: e.target.value})} required/></div><div><Label>قیمت (ریال)</Label><Input type="number" value={newProduct.price} onChange={e => setNewProduct({...newProduct, price: e.target.value})} required/></div><div><Label>آدرس عکس</Label><Input value={newProduct.image} onChange={e => setNewProduct({...newProduct, image: e.target.value})} /></div><div><Label>دسته‌بندی</Label><Select onValueChange={val => setNewProduct({...newProduct, categoryId: val})} required><SelectTrigger><SelectValue placeholder="انتخاب کنید" /></SelectTrigger><SelectContent>{categories.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div><div><Label>توضیحات</Label><Textarea value={newProduct.description} onChange={e => setNewProduct({...newProduct, description: e.target.value})} /></div><DialogFooter><DialogClose asChild id="add-product-close"><Button type="button" variant="secondary">انصراف</Button></DialogClose><Button type="submit" disabled={actionLoading === 'new'}>{actionLoading === 'new' ? <Loader2 className="animate-spin" /> : "ذخیره"}</Button></DialogFooter></form></DialogContent></Dialog></div>
            <Card><CardContent className="mt-6"><Table><TableHeader><TableRow><TableHead className="text-right w-[80px]">عکس</TableHead><TableHead className="text-right">نام</TableHead><TableHead className="text-right">دسته‌بندی</TableHead><TableHead className="text-right">قیمت</TableHead><TableHead className="text-right">وضعیت</TableHead><TableHead className="text-right">تغییر وضعیت</TableHead><TableHead className="text-right">عملیات</TableHead></TableRow></TableHeader><TableBody>{products.map(p => (<TableRow key={p.id}><TableCell className="text-right"><img src={p.image || "/placeholder.svg"} alt={p.name} className="h-12 w-12 rounded-md object-cover" /></TableCell><TableCell className="text-right font-medium">{p.name}</TableCell><TableCell className="text-right text-muted-foreground">{p.category.name}</TableCell><TableCell className="text-right">{p.price.toLocaleString('fa-IR')} ریال</TableCell><TableCell className="text-right"><Badge variant={p.available ? 'default' : 'destructive'}>{p.available ? "موجود" : "ناموجود"}</Badge></TableCell><TableCell className="text-right">{actionLoading === p.id ? <Loader2 className="h-5 w-5 animate-spin" /> : <Switch checked={p.available} onCheckedChange={() => handleToggleAvailability(p)} />}</TableCell><TableCell className="text-right"><Button size="sm" variant="outline" onClick={() => setEditingProduct(p)}><Pencil className="h-4 w-4" /></Button></TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
            <Dialog open={!!editingProduct} onOpenChange={() => setEditingProduct(null)}><DialogContent dir="rtl" className="sm:max-w-[425px]"><DialogHeader><DialogTitle>ویرایش محصول: {editingProduct?.name}</DialogTitle></DialogHeader>{editingProduct && (<form onSubmit={handleProductUpdate} className="space-y-4 pt-4"><div><Label>نام محصول</Label><Input value={editingProduct.name} onChange={e => setEditingProduct({...editingProduct, name: e.target.value})} required/></div><div><Label>قیمت (ریال)</Label><Input type="number" value={editingProduct.price} onChange={e => setEditingProduct({...editingProduct, price: parseFloat(e.target.value) || 0})} required/></div><div><Label>آدرس URL عکس</Label><Input value={editingProduct.image || ""} onChange={e => setEditingProduct({...editingProduct, image: e.target.value})} /></div><div><Label>دسته‌بندی</Label><Select value={editingProduct.categoryId} onValueChange={val => setEditingProduct({...editingProduct, categoryId: val})} required><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{categories.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div><div><Label>توضیحات</Label><Textarea value={editingProduct.description || ""} onChange={e => setEditingProduct({...editingProduct, description: e.target.value})} /></div><DialogFooter><Button type="button" variant="secondary" onClick={() => setEditingProduct(null)}>انصراف</Button><Button type="submit" disabled={actionLoading === editingProduct.id}>{actionLoading === editingProduct.id ? <Loader2 className="animate-spin" /> : "ذخیره تغییرات"}</Button></DialogFooter></form>)}</DialogContent></Dialog>
        </div>
    );
}
...
import { useState, useEffect, useMemo, FormEvent } from "react";
import { DollarSign, ShoppingCart, Users, LogOut, Package, Send, LayoutDashboard, ListPlus, PlusCircle, Pencil, Loader2, RefreshCw } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from "recharts";
import { useAppContext } from "@/context/AppContext";
import type { OrderWithItems, FullReturnRequest } from "@/types";
import type { Category, Product, ReturnRequest } from "@prisma/client";
...
export default function AdminDashboardLayout() {
    ...
    const renderActivePage = () => {
        switch (activePage) {
            case 'products': return <ProductManagementPage />;
            case 'categories': return <CategoryManagementPage />;
            case 'returns': return <ReturnManagementPage />;
            default: return <DashboardHomePage />;
        }
    };
            ...
            <aside className="w-64 bg-card border-l p-4 flex flex-col justify-between">
                <div>
                    ...
                    <nav className="space-y-2">
                        ...
                        <Button variant={activePage === 'products' ? 'secondary' : 'ghost'} className="w-full justify-start gap-2" onClick={() => setActivePage('products')}><Package /> مدیریت محصولات</Button>
                        <Button variant={activePage === 'categories' ? 'secondary' : 'ghost'} className="w-full justify-start gap-2" onClick={() => setActivePage('categories')}><ListPlus /> مدیریت دسته‌بندی‌ها</Button>
                        <Button variant={activePage === 'returns' ? 'secondary' : 'ghost'} className="w-full justify-start gap-2" onClick={() => setActivePage('returns')}><RefreshCw /> مدیریت مرجوعی‌ها</Button>
                    </nav>
                </div>
                <Button variant="ghost" onClick={handleLogout} className="w-full justify-start gap-2 text-destructive hover:text-destructive"><LogOut /> خروج</Button>
            </aside>
...
// --- Component for Dashboard Home (No changes needed here) ---
function DashboardHomePage() {
    // ... Existing code for dashboard home ...
    return <div>داشبورد اصلی</div>
}

// --- Component for Product Management (With Discount Field) ---
function ProductManagementPage() {
    const [products, setProducts] = useState<(Product & { category: Category })[]>([]);
    const [categories, setCategories] = useState<Category[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    ...
    return (
        <div>
            ...
            <Card><CardContent className="mt-6"><Table><TableHeader><TableRow><TableHead>عکس</TableHead><TableHead>نام</TableHead><TableHead>قیمت</TableHead><TableHead>تخفیف</TableHead><TableHead>وضعیت</TableHead><TableHead>عملیات</TableHead></TableRow></TableHeader><TableBody>{products.map(p => (<TableRow key={p.id}><TableCell><img src={p.image || "/placeholder.svg"} alt={p.name} className="h-12 w-12 rounded-md object-cover" /></TableCell><TableCell className="font-medium">{p.name}</TableCell><TableCell>{p.price.toLocaleString('fa-IR')} ریال</TableCell><TableCell>{p.discountPercentage > 0 ? `${p.discountPercentage}%` : '-'}</TableCell><TableCell><Badge variant={p.available ? 'default' : 'destructive'}>{p.available ? "موجود" : "ناموجود"}</Badge></TableCell><TableCell><Button size="sm" variant="outline" onClick={() => setEditingProduct(p)}><Pencil className="h-4 w-4" /></Button></TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
...
// --- Component for Category Management (No changes needed) ---
function CategoryManagementPage() {
    // ... Existing code for category management ...
     return <div>مدیریت دسته بندی</div>
}

// --- NEW Component for Return Management ---
function ReturnManagementPage() {
    const [returns, setReturns] = useState<FullReturnRequest[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [actionLoading, setActionLoading] = useState<string | null>(null);
    ...
    const handleStatusUpdate = async (returnId: string, status: 'APPROVED' | 'REJECTED') => {
        setActionLoading(returnId);
        try {
            await fetch(`/api/admin/returns/${returnId}/status`, {
                method: 'PATCH',
    ...
    if (isLoading) return <p className="text-center py-10">در حال بارگذاری درخواست‌های مرجوعی...</p>;

    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold">مدیریت مرجوعی‌ها</h1>
            {returns.length === 0 ? (
                <p className="text-center py-10 text-muted-foreground">هیچ درخواست مرجوعی ثبت نشده است.</p>
            ) : (
                <Card>
                    <CardContent className="mt-6">
                        <Table>
                            ...
                            <TableBody>
                                {returns.map(req => (
                                    <TableRow key={req.id}>
                                        <TableCell>...{req.orderId.slice(-6)}</TableCell>
                                        <TableCell>{req.order.user.shopName || req.order.user.name}</TableCell>
                                        ...
                                        <TableCell className="space-x-2 space-x-reverse">
                                            {req.status === 'REQUESTED' && (
                                                <>
                                                    <Button size="sm" disabled={!!actionLoading} onClick={() => handleStatusUpdate(req.id, 'APPROVED')}>تایید</Button>
                                                    <Button size="sm" variant="destructive" disabled={!!actionLoading} onClick={() => handleStatusUpdate(req.id, 'REJECTED')}>رد</Button>
                                                </>
                                            )}
                                        </TableCell>
                                    </TableRow>
